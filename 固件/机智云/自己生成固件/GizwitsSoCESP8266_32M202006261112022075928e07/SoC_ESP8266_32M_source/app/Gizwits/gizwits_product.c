...
int8_t ICACHE_FLASH_ATTR gizwitsEventProcess(eventInfo_t *info, uint8_t *data, uint32_t len)
{
    ...
    for(i = 0; i < info->num; i++)
    {
        switch(info->event[i])
        {
        case EVENT_ESP_01S_switch :
            currentDataPoint.valueESP_01S_switch = dataPointPtr->valueESP_01S_switch;
            GIZWITS_LOG("Evt: EVENT_ESP_01S_switch %d \n", currentDataPoint.valueESP_01S_switch);
            if(0x01 == currentDataPoint.valueESP_01S_switch)
            {
                //user handle
            }
            else
