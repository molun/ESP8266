...
char auth[] = "3c02e88bc6f8";   // 点灯平台设备密钥（Secret Key）
//char ssid[] = "tymishop";   // 你的wifi名称，使用APCONFIG或者SMARTCONFIG配网时注释掉
//char pswd[] = "15581822425";   // 你的wifi密码，使用APCONFIG或者SMARTCONFIG配网时注释掉
...
void setup()
{
    ...
    Blinker.begin(auth);   // 写入固定wifi名称和密码配网时注释掉
//    Blinker.begin(auth, ssid, pswd);   // 使用APCONFIG或者SMARTCONFIG配网时注释掉
    Blinker.attachData(dataRead); 
    Blinker.attachHeartbeat(heartbeat);   // 注册心跳包回调函数
    BlinkerMIOT.attachQuery(miotQuery);   // 注册小爱设备查询接口回调函数    
    Blinker.setTimezone(8.0);   // 设置时区为东八区（北京时间）
