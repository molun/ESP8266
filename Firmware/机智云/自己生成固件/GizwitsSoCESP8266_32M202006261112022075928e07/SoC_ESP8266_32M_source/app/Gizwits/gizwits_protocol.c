...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagESP_01S_switch)
    {
        info->event[info->num] = EVENT_ESP_01S_switch;
        info->num++;
        dataPoints->valueESP_01S_switch = gizStandardDecompressionValue(ESP_01S_switch_BYTEOFFSET,ESP_01S_switch_BITOFFSET,ESP_01S_switch_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueESP_01S_switch != cur->valueESP_01S_switch)
    {
        GIZWITS_LOG("valueESP_01S_switch Changed\n");
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(ESP_01S_switch_BYTEOFFSET,ESP_01S_switch_BITOFFSET,ESP_01S_switch_LEN,(uint8_t *)devStatusPtr,dataPoints->valueESP_01S_switch);
