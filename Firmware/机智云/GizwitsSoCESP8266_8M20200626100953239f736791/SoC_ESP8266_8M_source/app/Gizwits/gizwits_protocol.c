...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagOnOff)
    {
        ...
        info->num++;
        dataPoints->valueOnOff = gizStandardDecompressionValue(OnOff_BYTEOFFSET,OnOff_BITOFFSET,OnOff_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    {
        GIZWITS_LOG("valueOnOff Changed\n");
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(OnOff_BYTEOFFSET,OnOff_BITOFFSET,OnOff_LEN,(uint8_t *)devStatusPtr,dataPoints->valueOnOff);
